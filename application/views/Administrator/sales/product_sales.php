					...
					<div class="row">
						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"> Ton/Sheet </label>
									<div class="col-sm-4">
										<input type="number" min="0" id="quantity" placeholder="Ton/Sheet" class="form-control" ref="quantity" v-model="selectedProduct.tonQty" v-on:input="productTotal" autocomplete="off" />
									</div>
									<div class="col-sm-1">Pcs</div>
									<div class="col-sm-4">
										<input type="text" min="0" class="form-control" v-model="selectedProduct.pcs" placeholder="Pcs" v-on:input="productTotal" required>
									</div>
								</div>
		...
		methods:{
			...
			async productColorChange() {
				if((this.selectedProduct.Product_SlNo != '' || this.selectedProduct.Product_SlNo != 0) && this.selectedColor != null && this.sales.isService == 'false'){
					await axios.post('/get_product_color_stock', {productId: this.selectedProduct.Product_SlNo, colorId: this.selectedColor.color_id})
					.then(res => {
			...
			async productSizeChange() {
				if((this.selectedProduct.Product_SlNo != '' || this.selectedProduct.Product_SlNo != 0) && this.selectedColor != null && this.selectedSize != null && this.sales.isService == 'false'){
					await axios.post('/get_product_size_stock', {productId: this.selectedProduct.Product_SlNo, colorId: this.selectedColor.color_id, sizeId: this.selectedSize.size_id})
					.then(res => {
						this.productStock = res.data;
						// console.log(this.productStock);
					})
