					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:center;">
								...
								<?php if($this->session->userdata('accountType') != 'u'){?>
								<a href="javascript:" title="Edit Order" @click="checkReturnAndEdit(sale)"><i class="fa fa-edit"></i></a>
								<a href="" :style='{display: sale.Status=="p"?"":"none"}' title="Order Delivered" @click.prevent="deliveryOrder(sale.SaleMaster_SlNo)"><i class="fa fa-truck"></i></a>
								<a href="" title="Delete Order" @click.prevent="deleteSale(sale.SaleMaster_SlNo)"><i class="fa fa-trash"></i></a>
				...
				.then(res => {
					if(this.recordType == 'with_details'){
						if(this.EmployeeID == '') {
							...
							this.sales = res.data.filter(item => item.employee_id == this.EmployeeID)
						}
					} else {
						if(this.EmployeeID == '') {
