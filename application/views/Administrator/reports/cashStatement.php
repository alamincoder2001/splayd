...
<script>
	new Vue({
		...
		computed: {
			...
			totalCommission() {
				return this.commissions.reduce((prev, curr) => {
			...
			totalCashIn(){
				return parseFloat(this.totalSales) + 
					...
					parseFloat(this.totalCommission) + 
		...
		methods: {
			...
			getCommissionWithdraw() {
				axios.post('/get_commission_withdraw', this.filter).then(res => {
