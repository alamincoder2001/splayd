    ...
    public function addProduct()
    {
        ...
        try {
            ...
            $product = array(
                ...
                'Product_Purchase_Rate' => $data->product->Product_Purchase_Rate,
                'Product_SellingPrice' => $data->product->Product_SellingPrice,
                'Product_WholesaleRate' => $data->product->Product_WholesaleRate,
                'Unit_ID' => $data->product->Unit_ID,
                'per_unit_convert' => $data->product->per_unit_convert,
                'is_service' => $data->product->is_service == true ? 'true' : 'false',
                'status' => 'a',
                'AddBy' => $this->session->userdata("FullName"),
                'AddTime' => date('Y-m-d H:i:s'),
                'Product_branchid' => $this->brunch
            );
    ...
    public function getProductColor()
    {
        ...
        $colors = $this->db->query("
            select 
                ...
                (select (p.Product_Purchase_Rate * pc.stock)) as stock_value
            from tbl_color_size pc
            join tbl_color c on c.color_SiNo = pc.color_id
    ...
    public function getProductSize()
    {
        ...
        $colors = $this->db->query("
            select 
                ...
                (select (p.Product_Purchase_Rate * pc.stock)) as stock_value
            from tbl_color_size pc
            join tbl_size s on s.size_SiNo = pc.size_id
            join tbl_product p on p.Product_SlNo = pc.product_id
            where pc.status = 'a'
            and pc.product_id = ?
            and pc.color_id = ?
