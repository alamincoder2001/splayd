    ...
    public function getSales(){
        ...
        if(isset($data->salesId) && $data->salesId != 0 && $data->salesId != ''){
            ...
            $saleDetails = array_map(function($detail) {
                $detail->quantity_text = floor($detail->SaleDetails_TotalQuantity / $detail->per_unit_convert) .' Ton/Sheet ' . $detail->SaleDetails_TotalQuantity % $detail->per_unit_convert . ' '. $detail->Unit_Name;
                return $detail;
            }, $saleDetails);
        }
        $sales = $this->db->query("
        ...
        try{
            ...
            foreach($currentSaleDetails as $product){
                ...
                $this->db->query("
                    ...
                    and size_id = ? 
                    and branch_id = ?
                ",[$product->SaleDetails_TotalQuantity, $product->Product_IDNo, $product->Product_colorId, $product->Product_sizeId, $this->session->userdata('BRANCHid')]);
            }
        ...
        try{
            ...
            foreach($data->cart as $product){
                $returnDetails = array(
                    ...
                    'SaleReturnDetailsColorId' => $product->Product_colorId,
                ...
                $this->db->query("
                    ...
                    and size_id = ? 
                    and branch_id = ?
                ",[$product->return_quantity, $product->Product_IDNo, $product->Product_colorId, $product->Product_sizeId, $this->session->userdata('BRANCHid')]);
            }
        ...
        try{
            ...
            foreach($oldDetails as $product) {
                ...
                $this->db->query("
                    ...
                    and size_id = ? 
                    and branch_id = ?
                ",[$product->SaleReturnDetails_ReturnQuantity, $product->SaleReturnDetailsProduct_SlNo, $product->SaleReturnDetailsColorId, $product->SaleReturnDetailsSizeId, $this->session->userdata('BRANCHid')]);
            }
    ...
    public function getSaleReturns() {
        ...
        if(isset($data->id) && $data->id != '') {
            ...
            $res['returnDetails'] = $this->db->query("
                ...
                left join tbl_color c on c.color_SiNo = srd.SaleReturnDetailsColorId
                left join tbl_size s on s.size_SiNo = srd.SaleReturnDetailsSizeId
