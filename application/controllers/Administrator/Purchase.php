    ...
    public function getPurchases(){
        ...
        if(isset($data->purchaseId) && $data->purchaseId != null){
            ...
            $purchaseDetails = $this->db->query("
                ...
                left join tbl_color c on c.color_SiNo = pd.Product_colorId
                left join tbl_size s on s.size_SiNo = pd.Product_sizeId
        ...
        try{
            ...
            foreach($oldDetails as $product) {
                ...
                $this->db->query("
                    ...
                    and size_id = ? 
                    and branch_id = ?
                ",[$product->PurchaseReturnDetails_ReturnQuantity, $product->PurchaseReturnDetailsProduct_SlNo, $product->PurchaseReturnDetailsColorId, $product->PurchaseReturnDetailsSizeId, $this->session->userdata('BRANCHid')]);
            }
    ...
    public function getPurchaseReturns() {
        ...
        if(isset($data->id) && $data->id != '') {
            ...
            $res['returnDetails'] = $this->db->query("
                ...
                from tbl_purchasereturndetails prd
                join tbl_product p on p.Product_SlNo = prd.PurchaseReturnDetailsProduct_SlNo
                left join tbl_color c on c.color_SiNo = prd.PurchaseReturnDetailsColorId
                left join tbl_size s on s.size_SiNo = prd.PurchaseReturnDetailsSizeId
