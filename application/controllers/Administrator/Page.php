...
// ) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
    ...
    public function getSizes() {
        $colors = $this->db->query("
            ...
            from tbl_size s
            where s.status = 'a'
            order by s.size_SiNo asc
        ")->result();

        echo json_encode($colors);
    }
    public function Fancybox_add_size(){
        $data['title'] = "Add size";
        $this->load->view('Administrator/products/fancybox_size', $data);
        
    }
    public function insert_size()  {
        $sizename = $this->input->post('sizename');
        $query = $this->db->query("SELECT size_name from tbl_size where size_name = '$sizename'");
        ...
        if($query->num_rows() > 0){
            ...
            $data = array(
                "size_name"      =>$this->input->post('sizename', TRUE),
                "status"          =>'a',
                "add_date"          => date('Y-m-d')
                
                );
            if($this->mt->save_data('tbl_size',$data))
            {
                    $msg = true;
                echo json_encode($msg);
            }
    ...
    public function fancybox_insert_size()  {
        $mail = $this->input->post('Country');
        $query = $this->db->query("SELECT size_name from tbl_size where size_name = '$mail'");
        ...
        if($query->num_rows() > 0){
            ...
            $data = array(
                "size_name"          =>$this->input->post('Country', TRUE)
                );
            $this->mt->save_data('tbl_size',$data);
            $this->load->view('Administrator/products/ajax_size');
        }
    }
    public function sizedelete(){
        $id = $this->input->post('deleted');
        $fld = 'size_SiNo';
        $this->mt->delete_data("tbl_size", $id, $fld);
        echo "Success";
    }
    public function sizeedit($id){
        $data['title'] = "Edit size";
        $fld = 'size_SiNo';
        $data['selected'] = $this->Billing_model->select_by_id('tbl_size', $id,$fld);
        $data['content'] = $this->load->view('Administrator/edit/edit_size', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function sizeupdate(){
        ...
        $fld = 'size_SiNo';
            ...
            if($this->mt->update_data("tbl_size", $data, $id,$fld)){
                    $msg = true;
                    echo json_encode($msg);
            }
        }
